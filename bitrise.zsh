...
function get() {
  ...
  for build in $builds; do
    ...
    if [[ "$build_status" -ne 0 ]]; then
      echo -e "Build $build_number - $(print_status_text $build_status)"
    else
      echo -e "Build $build_number - $(print_status_text $build_status) - Time elapsed: $time_elapsed"
    fi
  done
  return 0
}
...
function message_helper() {
  ...
  echo -e "${color_code}${status_type}: ${message}\033[0m"
}
